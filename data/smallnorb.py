'''
    This script is borrowed here: https://github.com/pytorch/vision/pull/492/files
'''
...
class SmallNORB(data.Dataset):
    ...
    """
    The output shuld be a tuple:
        (image, target, info)
    The output "target" contains the object category in images:
        (0 for animal, 1 for human, 2 for plane, 3 for truck, 4 for car).
    The output "info" contains the factors of images:
    ...
    def __init__(self, root, img_size=64, train=True, transform=None, target_transform=None, info_transform=None, download=True,
                 mode="all"):
        ...
        # load labels
        self.targets = self._load(label_file)
    ...
    def __getitem__(self, index):
        ...
        """
        target = self.targets[index % 24300] if self.mode == "all" else self.targets[index]
